...
class Chunk(BaseModel):
    ...
    section_title: str | None = Field(
        default=None, description="Section title from segment."
    )
    ...
    char_end: int | None = Field(
        default=None, description="End offset in segment text."
    )
...
class StructuralSegment(BaseModel):
    ...
    page_number: int = Field(..., ge=1, description="1-based page index.")
    section_title: str | None = Field(
        default=None, description="Section title from page."
    )
    article_numbers: list[str] = Field(
        default_factory=list,
    ...
    char_start: int | None = Field(
        default=None, description="Start offset in original page text."
    )
    char_end: int | None = Field(
        default=None, description="End offset in original page text."
    )
